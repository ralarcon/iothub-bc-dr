...
namespace ImportExportIotDevices
{
    public class ImportExportDevicesSample
    {
            ...
            bool shouldDeleteDestDevices)
        {
            ...
            if(shouldExportDevices)
            {
                // Export devices from the original hub to the storage account
                await ExportHubDevicesToStorageAsync(srcRegistryManager, destRegistryManager, includeConfigurations);
            }


            if (shouldCopyDevices && !shouldExportDevices)
            {
            ...
            if (shouldDeleteSourceDevices)
            {
                ...
                Console.WriteLine("DELETE SOURCE DEVICES NOT IMPLEMENTED. EXPLICITLY COMMENTED IN SOURCE CODE.");
            }
        ...
        /// <param name="sourceHubConnectionString">Connection string for source hub.</param>
        /// <param name="destHubConnectionString">Connection string for destination hub.</param>
        private async Task ExportHubDevicesToStorageAsync(RegistryManager srcRegistryManager, RegistryManager destRegistryManager, bool includeConfigurations)
        {
            Console.WriteLine("Copying devices from the source to the destination IoT hub.");
            var stopwatch = Stopwatch.StartNew();
            Console.WriteLine("Exporting devices to destination IoT hub.");
