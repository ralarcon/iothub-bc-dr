...
namespace ImportExportIotDevices
{
    ...
    public class Parameters
    {
        ...
        [Option(
            "ExportDevices",
            Default = false,
            HelpText = "Export devices (and configurations, if specified) from the source to the storage account. Defaults to environment variable 'EXPORT_DEVICES'.")]
        public bool ExportDevices { get; set; }
        ...
        public Parameters()
        {
            ...
            string exportDevicesEnv = Environment.GetEnvironmentVariable("EXPORT_DEVICES");
            if (!string.IsNullOrWhiteSpace(exportDevicesEnv)
                && bool.TryParse(exportDevicesEnv, out bool exportDevices))
            {
